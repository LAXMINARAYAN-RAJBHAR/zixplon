...
// ── Auto-generated username detector ─────────────────────────────────────────
// Supabase creates usernames like "user_4f16d7ad" when no real name exists.
// We treat these as "no username set" and prefer real metadata instead.
const AUTO_USERNAME_RE = /^user_[a-f0-9]{6,}$/i;
const isAutoGenerated = (name) => !name || AUTO_USERNAME_RE.test(name.trim());
...
function App() {
  ...
  // ── Single auth effect — profiles table is source of truth ──
  useEffect(() => {
    const resolveUsername = async (u) => {
      try {
        ...
        // Best real name from OAuth metadata (Google gives full_name / name)
        const metaName =
        ...
        // ── CASE 1: No profile row at all → create one with real name ──
        if (!profileRow) {
          ...
          _persistLocally(u, metaName, metaPic, u.user_metadata?.about || "");
          return metaName;
        }

        // ── CASE 2: Profile exists but username is auto-generated (user_xxxxxxxx)
        //    → update the profile row with the real name from metadata ──
        const storedUsername = profileRow.username;
        let finalName;

        if (isAutoGenerated(storedUsername) && metaName) {
          // Update Supabase so it's fixed for all future sessions
          await supabase
            .from("profiles")
            .update({ username: metaName })
            .eq("id", u.id);

          finalName = metaName;
        } else {
          // Profile has a real username — use it
          finalName = storedUsername || metaName;
        }
        ...
        _persistLocally(u, finalName, pic, about);
        return finalName;

      } catch (e) {
        console.error("resolveUsername error:", e);
        // Fallback: whatever we can find
        return (
    ...
    // Helper: write all auth keys to localStorage consistently
    const _persistLocally = (u, name, pic, about) => {
    ...
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      if (session?.user) {
        ...
        setCurrentUser(name);
        // Clean up OAuth hash from URL bar
