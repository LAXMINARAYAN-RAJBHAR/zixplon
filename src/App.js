...
// ── Detect auto-generated usernames (user_<hex8>) ────────────────────────────
const isAutoGenerated = (name) =>
  !name || /^user_[0-9a-f]{6,}$/i.test(name.trim());

// ── Loading Screen ────────────────────────────────────────────────────────────
const LoadingScreen = ({ onFinish, authReady }) => {
  const [fade, setFade] = useState(false);
  // FIX: only start fade-out once BOTH the timer AND auth are resolved.
  // This prevents the navbar flashing "Sign In" while Supabase session loads.
  const minTimerDone = useRef(false);

  useEffect(() => {
    const fadeTimer = setTimeout(() => {
      minTimerDone.current = true;
      // If auth is already done, fade immediately
      if (authReady) setFade(true);
    }, 1800);

    return () => clearTimeout(fadeTimer);
  }, []);

  // When authReady flips to true (after session check), trigger fade if min time passed
  useEffect(() => {
    if (authReady && minTimerDone.current) {
      setFade(true);
    }
  }, [authReady]);

  // Once fade animation finishes, unmount
  useEffect(() => {
    if (!fade) return;
    const doneTimer = setTimeout(() => onFinish && onFinish(), 500);
  ...
  return (
    ...
    >
      ...
      {/* Auth status hint — shows while session is being checked */}
      <p
        style={{
          color: authReady ? "#4caf50" : "#888",
          fontSize: "12px",
      ...
      >
        {authReady ? "✓ Session restored" : "Checking session..."}
      </p>
...
function App() {
  ...
  const [authReady, setAuthReady]   = useState(false); // NEW: tracks when session check is done
  ...
  // loginModalOpenRef — prevents onAuthStateChange from firing while modal is open
  ...
  // ── Auth: resolveUsername ─────────────────────────────────────────────────
  // Fetches the full profile from DB (including channel_name), writes all keys
  // to localStorage, and returns the resolved display-friendly username.
  const resolveUsername = async (u) => {
    try {
      const { data: profileRow } = await supabase
        .from("profiles")
        .select("id, username, channel_name, profile_pic, about, banner_pic")
        .eq("id", u.id)
        .maybeSingle();

      // ── Auto-create profile for Google OAuth / new users who have none ──
      if (!profileRow) {
        const autoName =
        ...
        await supabase.from("profiles").upsert(
          [{
            id:           u.id,
            username:     autoName,
            channel_name: autoName,   // default channel_name same as username
            profile_pic:  autoPic,
            about:        "",
            banner_pic:   "",
          }],
        ...
        localStorage.setItem("channelName", autoName);
        localStorage.setItem("email",       u.email || "");
        if (autoPic) localStorage.setItem("profilePic", autoPic);
        return autoName;
      }

      // ── Profile exists — resolve each field with priority chain ──────────

      // username: DB row → localStorage (only if not auto-generated) → email prefix
      const dbUsername = profileRow?.username;
      const lsUsername = localStorage.getItem("username");
      const name =
        (!isAutoGenerated(dbUsername) ? dbUsername : null) ||
        (!isAutoGenerated(lsUsername) ? lsUsername : null) ||
        u.user_metadata?.channelName ||
        u.user_metadata?.username ||
        u.user_metadata?.full_name ||
        u.email?.split("@")[0];

      // channel_name: DB row → auth metadata → localStorage → username
      const channelName =
        profileRow?.channel_name ||
        u.user_metadata?.channel_name ||
        localStorage.getItem("channelName") ||
        name;

      // profile_pic: DB row → localStorage → auth metadata
      const pic =
      ...
      // about: DB row → localStorage → auth metadata
      const about =
        profileRow?.about ||
      ...
      // ── Write everything to localStorage so it's available synchronously ──
      localStorage.setItem("userId",      u.id);
  ...
  useEffect(() => {
    ...
    // STEP 1: Check for existing session on mount (auto-login detection)
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      // Signal that session check is done — loading screen can now finish
      if (mounted) setAuthReady(true);
    });

    // STEP 2: Keep listening for login / logout events
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (_event, session) => {
        // Guard: don't fire while the login modal is actively open
        // (prevents remounting the modal mid-typing on mobile)
        ...
        // User signed in or token refreshed — restore full profile
        const name = await resolveUsername(session.user);
        if (mounted) setCurrentUser(name);
      }
    );
  ...
  // ── Render loading screen until auth check is done ────────────────────────
  if (!appReady) {
    return (
      <LoadingScreen
        authReady={authReady}
  ...
  return (
    ...
    >
      ...
      <Navbar
        ...
        loginModalOpenRef={loginModalOpenRef}  // pass ref so Navbar can set it
      />
