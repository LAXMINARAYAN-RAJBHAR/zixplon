  ...
  return (
    <div className="homePage">
      <div className={"homePage_options" + (sideNavbar ? " sidebar-open" : "")}>
        ...
        >
          {options.map((item) => (
            <div key={item} className="homePage_option" onClick={() => setSelectedOption(item)} style={{ cursor: "pointer", background: selectedOption === item ? "white" : "transparent", color: selectedOption === item ? "black" : "white", borderRadius: "8px", padding: "6px 12px", fontWeight: selectedOption === item ? "600" : "400", transition: "all 0.2s", whiteSpace: "nowrap", userSelect: "none" }}>{item}</div>
          ))}
        </div>
      </div>
                  ...
                  <React.Fragment key={rowIndex}>
                    ...
                    <div className="youtube_VideoGrid">
                      ...
                      {allVids.slice(rowIndex * 12, rowIndex * 12 + 12).map((v) => (
                        <VideoCard key={v.id} video={v} isUploaded={v.isUploaded || false} />
                      ))}
                    </div>
                  </React.Fragment>,
                );
