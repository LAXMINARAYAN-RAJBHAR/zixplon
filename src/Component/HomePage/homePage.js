...
// ── FIX 1: Three dots on RIGHT side, hidden on mobile ──
// ── FIX 2: Watch Later is always sticky/visible even with many playlists ──
const SaveMenuButton = ({ videoId, isSaved, onToggleWatchLater, playlistsCache, setPlaylistsCache }) => {
  ...
  return (
    <>
      {/*
        ── FIX 1: right: "8px" (not left) ──
        ── FIX 1: className="save-menu-wrapper" so CSS can hide it on mobile ──
      */}
      <div
        ref={wrapperRef}
      ...
      {open && (
        ...
        >
          {/*
            ── FIX 2: Sheet split into sticky header + scrollable body ──
            Watch Later is always pinned at top and never scrolls away
          */}
          <div
            className="save-menu-sheet"
            onClick={(e) => e.stopPropagation()}
            style={{
              width: "100%",
              maxWidth: "480px",
          ...
          >
            {/* ── STICKY TOP: drag handle + title + Watch Later (never scrolls) ── */}
            <div style={{ flexShrink: 0, padding: "10px 18px 0" }}>
              ...
              {/* Watch Later — always visible, pinned */}
              <div
              ...
              >
                <span style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                  {isSaved
                    ...
                    : <BookmarkBorderIcon style={{ fontSize: 20, color: "#7c3aed" }} />
                  }
                  Watch Later
                </span>
              ...
              {/* Playlist label */}
              <div style={{ color: "#4c4589", fontSize: "13px", fontWeight: "800", padding: "2px 10px 8px", display: "flex", alignItems: "center", gap: "6px" }}>
                <PlaylistAddIcon style={{ fontSize: 16 }} /> Save to playlist
              </div>
            </div>

            {/* ── SCROLLABLE BODY: only playlists + new playlist scroll ── */}
            <div style={{ flex: 1, overflowY: "auto", padding: "0 18px", scrollbarWidth: "thin", scrollbarColor: "#e0d4ff transparent" }}>
              {loading && (
              ...
              {/* New playlist */}
              <div style={{ borderTop: "1px solid #f0ebff", marginTop: "10px", paddingTop: "12px", paddingBottom: "16px" }}>
                {!creating ? (
                  <div
  ...
  const VideoCard = ({ video, isUploaded = false, showDelete = false }) => {
    ...
    return (
      <div className="youtube_thumbnailBox" style={{ position:"relative" }}>
        {/* Delete button — right side, only for owner */}
        {isOwner && (
        ...
        {/* Three dots — right side on desktop, hidden on mobile via CSS */}
        {isUploaded && !isOwner && (
          <SaveMenuButton
            videoId={video.id}
            isSaved={isSaved}
