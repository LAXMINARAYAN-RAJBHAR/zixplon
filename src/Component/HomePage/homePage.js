    ...
    <div style={{ padding: isMobile ? "0 12px" : "0" }}>
      ...
      <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginTop: "12px" }}>
        <div style={{ display: "flex", background: "#272727", borderRadius: "20px", overflow: "hidden" }}>
          <button onClick={() => { setIsLiked((l) => !l); if (isDisliked) setIsDisliked(false); }} style={{ background: isLiked ? "#3ea6ff22" : "transparent", border: "none", color: isLiked ? "#3ea6ff" : "white", padding: "8px 16px", cursor: "pointer", fontSize: "14px", borderRight: "1px solid #3a3a3a" }}>
      ...
      <div style={{ position: "fixed", top: "55px", left: "0", right: "0", bottom: "0", zIndex: 999, background: "#0f0f0f", display: "flex", flexDirection: "column", fontFamily: "'Segoe UI', sans-serif", overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
          <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px", minWidth: 0, scrollbarWidth: "thin", scrollbarColor: "#555 transparent" }}>
            <Player /><NavBar /><MetaSection /><CommentSection />
          </div>
...
const HomePage = ({ sideNavbar }) => {
  ...
  const options = [
    ...
    "Film Criticisms", "Twenty20 Cricket", "Music", "Live", "Mixes", "Gaming", "Debates",
    "Coke Studio India", "Democracy", "Pakistani Dramas", "Pakistani Movies", "Comedy",
    "Podcasts", "WWE", "Superhero Movies", "Dramedy", "Web Development", "Hollywood Movies",
    "Dubbed Hollywood Movies", "Web Series", "Professional Wrestling", "Bhojpuri Cinema",
    "Bhojpuri Songs", "Astronomy", "AI", "History", "Geographical Videos",
    ...
    const subscription = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "videos" }, (payload) => {
        ...
        setDbVideos((prev) => [{
          ...
          channel: v.channel, tags: [v.category || "All"],
        }, ...prev]);
      })
      .subscribe();
  ...
  const openWatchPage  = (videoId, title, channel) => setWatchVideo({ videoId, title, channel });
      ...
      <div className={`home_mainPage ${sideNavbar ? "sidebar-open" : "sidebar-closed"}`}>
        ...
        {selectedOption === "All" ? (
          ...
          <div style={{ padding: "16px 20px" }}>
            ...
            {/* Uploaded videos matching category */}
            {dbVideos.filter((v) => v.tags?.includes(selectedOption)).length > 0 && (
              <div style={{ marginBottom: "40px" }}>
                ...
                <div className="youtube_VideoGrid">
                  {dbVideos.filter((v) => v.tags?.includes(selectedOption)).map((v) => <VideoCard key={v.id} video={v} isUploaded={true} />)}
                </div>
              </div>
            )}

            {/* Local hardcoded videos matching category */}
            {videos.filter((v) => v.tags?.includes(selectedOption)).length > 0 && (
              <div style={{ marginBottom: "40px" }}>
                ...
                <div className="youtube_VideoGrid">
                  {videos.filter((v) => v.tags?.includes(selectedOption)).map((v) => <VideoCard key={v.id} video={v} />)}
                </div>
              </div>
            )}
