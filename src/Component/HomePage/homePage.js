          ...
          {trendingVideos.map((v, i) => {
            ...
            return (
              ...
              >
                ...
                <span style={{ position: "absolute", top: 5, right: 5, background: badge.bg, color: "#fff", fontSize: "9px", fontWeight: "800", padding: "2px 5px", borderRadius: "4px", zIndex: 4 }}>
                  {badge.label}
                </span>
  ...
  return (
    ...
    >
      <div className="homePage_shortThumbnail">
        ...
        {showNew && (
          <div style={{ position: "absolute", top: "8px", left: "8px", background: "linear-gradient(135deg,#f43f5e,#f97316)", color: "white", fontSize: "10px", fontWeight: "800", padding: "2px 7px", borderRadius: "5px", zIndex: 2 }}>
            New
          </div>
        )}
        {short.dbId && (
...
// ─────────────────────────────────────────────────────────────────────────────
// SaveMenuButton — portal-based bottom sheet, fixed header + scrollable body
// ─────────────────────────────────────────────────────────────────────────────
const SaveMenuButton = ({ videoId, isSaved, onToggleWatchLater, playlistsCache, setPlaylistsCache }) => {
  ...
  useEffect(() => {
    if (!open) return;
    const handleClaim = (e) => { if (e.detail !== videoId) setOpen(false); };
  ...
  const closeSheet = () => { setOpen(false); setCreating(false); setNewTitle(""); };
  ...
  return (
    <>
      ...
      {/* Portal-based bottom sheet — renders directly on document.body */}
      {open && createPortal(
        <div
        ...
        >
          ...
          >
            {/* ── FIXED HEADER: drag pill + title + Watch Later + label ── */}
            <div style={{ padding: "10px 18px 0", flexShrink: 0 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "14px" }}>
                ...
                <button
                  onClick={closeSheet}
                  style={{ background: "#f7f0ff", border: "none", color: "#7c3aed", borderRadius: "50%", width: "30px", height: "30px", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}
                >
                  <CloseIcon style={{ fontSize: 18 }} />
                </button>
              </div>

              {/* Watch Later — pinned, never scrolls away */}
              <div
              ...
              <div style={{ color: "#4c4589", fontSize: "13px", fontWeight: "800", padding: "4px 10px 10px", display: "flex", alignItems: "center", gap: "6px", borderBottom: "1px solid #f0ebff" }}>
                <PlaylistAddIcon style={{ fontSize: 16 }} /> Save to playlist
              </div>
            </div>

            {/* ── SCROLLABLE BODY: playlist list + new playlist ── */}
            <div
              style={{ flex: 1, overflowY: "auto", WebkitOverflowScrolling: "touch", overscrollBehavior: "contain", padding: "4px 18px 22px" }}
            >
              {loading && (
                <div style={{ padding: "14px 10px", color: "#8b84c4", fontSize: "14px" }}>Loading…</div>
              )}
                ...
                {!creating ? (
                  ...
                  <div style={{ display: "flex", gap: "8px", padding: "4px 2px" }}>
                    ...
                    <button
                      ...
                      style={{ background: "#7c3aed", border: "none", color: "white", borderRadius: "10px", padding: "0 16px", fontSize: "14px", fontWeight: "700", cursor: "pointer" }}
                    >
                      Create
                    </button>
                  </div>
...
const WatchPage = ({ initialVideoId, initialTitle, initialChannel, onClose, suggestions, onIncrementView }) => {
  ...
  const handleSubscribe = (ch) => {
    setSubscribedChannels((prev) => { const next = new Set(prev); next.has(ch) ? next.delete(ch) : next.add(ch); return next; });
  };
    ...
    <div style={{ padding: isMobile ? "0 12px" : "0" }}>
      ...
      <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginTop: "12px" }}>
        <div style={{ display: "flex", background: WP.surface2, borderRadius: "20px", overflow: "hidden", border: "1.5px solid " + WP.border }}>
          <button onClick={() => { setIsLiked((l) => !l); if (isDisliked) setIsDisliked(false); }} style={{ background: isLiked ? "#ede9fe" : "transparent", border: "none", color: isLiked ? WP.primary : WP.text2, padding: "8px 16px", cursor: "pointer", fontSize: "14px", borderRight: "1px solid " + WP.border, fontWeight: "600" }}>
      ...
      <div style={{ display: "flex", gap: "12px", marginBottom: "24px" }}>
        ...
        <div style={{ flex: 1 }}>
          <input
            ...
            style={{ width: "100%", background: "transparent", border: "none", borderBottom: "2px solid " + WP.border, color: WP.text, fontSize: "14px", padding: "8px 0", outline: "none", boxSizing: "border-box", fontFamily: "Outfit, sans-serif" }}
...
const HomePage = ({ sideNavbar }) => {
  ...
  const searchQuery = (() => { const params = new URLSearchParams(location.search); return (params.get("q") || "").trim().toLowerCase(); })();
  ...
  const loadWatchedIds = React.useCallback(async () => {
    ...
    if (data) { const ids = new Set(data.map((r) => `${r.content_type}_${r.content_id}`)); setWatchedContentIds(ids); }
  }, []);
  ...
  useEffect(() => {
    ...
    return () => { document.removeEventListener("visibilitychange", handleVisibility); window.removeEventListener("focus", handleFocus); clearInterval(interval); };
  }, [loadWatchedIds]);
  ...
  useEffect(() => {
    ...
    const speed = 0.6;
    const step = () => {
      if (!isPausedRef.current) { pos += speed; if (pos >= track.scrollWidth - track.clientWidth) pos = 0; track.scrollLeft = pos; }
  ...
  const openWatchPage = (videoId, title, channel, dbVideoId = null) => { if (dbVideoId) incrementView(String(dbVideoId), "video"); setWatchVideo({ videoId, title, channel }); };
  ...
  const scoreVideo = (v) => {
    ...
    for (const w of words) { if (title.includes(w)) score += 4; if (tags.some((t) => t.includes(w))) score += 3; if (channel.includes(w)) score += 2; if (description.includes(w)) score += 1; }
    return score;
  };
  ...
  const VideoCard = ({ video, isUploaded = false, showDelete = false }) => {
    ...
    return (
      <div className="youtube_thumbnailBox" style={{ position: "relative" }}>
        {isOwner && (
          <button onClick={(e) => handleDeleteVideo(e, video.id)} title="Delete video" style={{ position: "absolute", top: "8px", right: "8px", zIndex: 10, background: "rgba(239,68,68,0.92)", border: "none", color: "white", borderRadius: "8px", padding: "4px 8px", cursor: "pointer", fontSize: "11px", fontWeight: "800", boxShadow: "0 2px 8px rgba(239,68,68,0.4)" }}>
            🗑 Delete
          </button>
        )}
        {isUploaded && (
          <SaveMenuButton videoId={video.id} isSaved={isSaved} onToggleWatchLater={handleToggleWatchLater} playlistsCache={playlistsCache} setPlaylistsCache={setPlaylistsCache} />
        )}
        <Link to={"/video/" + video.id} className="youtube_thumbnailWrapper" onClick={() => { if (isUploaded) incrementView(video.id, "video"); }}>
          ...
          {showNew && (
            <div style={{ position: "absolute", top: "8px", left: "38px", background: "linear-gradient(135deg,#f43f5e,#f97316)", color: "white", fontSize: "10px", fontWeight: "800", padding: "2px 7px", borderRadius: "5px", zIndex: 2 }}>New</div>
          )}
        ...
        <div className="youtubeTitleBox">
          <div className="youtubeBoxProfile">
            ...
            <Link to={"/user/" + (video.username || video.channel.toLowerCase())} style={{ textDecoration: "none", color: "inherit" }}>
          ...
          <div className="youtubeVideoInfo">
            ...
            <p className="youtubeVideo_Views" style={{ display: "flex", gap: "10px", alignItems: "center" }}>
              {isUploaded ? (
                <>
                  ...
                  <button onClick={(e) => handleLikeVideo(e, video.id)} style={{ background: "none", border: "none", color: "#8b84c4", cursor: "pointer", fontSize: "inherit", padding: 0, display: "flex", alignItems: "center", gap: "3px", fontWeight: "600" }}>
                    👍 {video.likes ?? 0} Likes
                  </button>
                </>
              ) : (<span>👍 3 Likes</span>)}
            ...
            {isUploaded && video.created_at && (
              <p className="youtubeVideo_UploadDate" style={{ color: "#a8a3d6", fontSize: "12px", margin: "3px 0 0", fontWeight: "500" }}>{formatTimeAgo(video.created_at)}</p>
            )}
          </div>
        </div>
      </div>
          ...
          <div className="mobile-tab-content" {...swipeHandlers}>
            {allReels.length === 0 ? (
              ...
              <>
                ...
                <div className="homePage_shortsRow">
                  {allReels.map((short) => (<ShortCard key={short.id} short={short} incrementView={incrementView} viewCounts={viewCounts} handleDeleteReel={handleDeleteReel} navigate={navigate} watchedContentIds={watchedContentIds} />))}
                </div>
              </>
            )}
          </div>
      ...
      case "videos":
        return (
          <div className="mobile-tab-content" {...swipeHandlers}>
            {dbVideos.length > 0 && (
              <>
                <div className="mobile-tab-section-head">Uploaded ({dbVideos.length})</div>
                <div className="youtube_VideoGrid" style={{ marginBottom: "16px" }}>
                  {dbVideos.map((v) => (<VideoCard key={v.id} video={v} isUploaded={true} showDelete={true} />))}
                </div>
              </>
            )}
      ...
      case "live":
        return (
          <div className="mobile-tab-content" {...swipeHandlers}>
            ...
            {liveVideos.length > 0 ? (
              <>
                <div className="mobile-tab-section-head">Live Videos ({liveVideos.length})</div>
                <div className="youtube_VideoGrid">{liveVideos.map((v) => (<VideoCard key={v.id} video={v} isUploaded={dbVideos.some((d) => d.id === v.id)} />))}</div>
              </>
            ) : (
              <div style={{ textAlign: "center", padding: "40px 0", color: "#8b84c4" }}>
                <div style={{ fontSize: "36px", marginBottom: "10px" }}>🔴</div>
  ...
  return (
    <div className="homePage">
      <div className={"homePage_options" + (sideNavbar ? " sidebar-open" : "")}>
        ...
        >
          {options.map((item) => (
            <div key={item} className="homePage_option" onClick={() => setSelectedOption(item)} style={{ cursor: "pointer", background: selectedOption === item ? "rgba(124,58,237,0.08)" : "transparent", color: selectedOption === item ? "#7c3aed" : "#4c4589", borderRadius: selectedOption === item ? "8px" : "0", padding: "6px 14px", fontWeight: selectedOption === item ? "800" : "600", transition: "all 0.2s", whiteSpace: "nowrap", userSelect: "none" }}>
              {item}
            </div>
          ))}
        </div>
      ...
      <div className={"home_mainPage" + (sideNavbar ? " sidebar-open" : " sidebar-closed")}>
        ...
        {!searchActive && <MobileTrendingStrip dbVideos={allVideos} dbReels={allReels} onVideoClick={(v) => navigate(`/video/${v.id}`)} onReelClick={(r) => navigate("/reels/" + r.id, { state: { clickedReel: r } })} />}
          ...
          <div style={{ padding: "16px 0" }}>
            ...
            {videos.filter((v) => v.tags?.includes(selectedOption)).length > 0 && (
              <div style={{ marginBottom: "40px" }}>
                ...
                <div className="youtube_VideoGrid">{videos.filter((v) => v.tags?.includes(selectedOption)).map((v) => (<VideoCard key={typeof v.id === "number" ? `static_${v.id}` : v.id} video={{ ...v, id: typeof v.id === "number" ? `static_${v.id}` : v.id }} />))}</div>
              </div>
            )}
            {ytLoading && (
              <div style={{ marginBottom: "40px" }}>
