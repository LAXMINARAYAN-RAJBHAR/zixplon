          ...
          {trendingVideos.map((v, i) => (
            ...
            >
              ...
              <div className="mobile-trending-card-info">
                ...
                <div className="mobile-trending-card-channel">{v.channel}</div>
              </div>
            </div>
          ))}
        </div>
...
const HomePage = ({ sideNavbar }) => {
  ...
  const searchedLocalVideos = searchActive
    ? [...dbVideos.map((v) => ({ ...v, isUploaded: true })), ...videos].map((v) => ({ ...v, _score: scoreVideo(v) })).filter((v) => v._score > 0).sort((a, b) => b._score - a._score)
    : [];
        ...
        return (
          <div className="mobile-tab-content">
            ...
            {movieVideos.length > 0 ? (
              <div className="youtube_VideoGrid">
                {movieVideos.map((v) => <VideoCard key={v.id} video={v} isUploaded={dbVideos.some((d) => d.id === v.id)} />)}
              </div>
            ) : (
              <div className="youtube_VideoGrid">
                {videos.map((v) => <VideoCard key={v.id} video={v} />)}
          ...
          <div className="mobile-tab-content">
            ...
            {liveVideos.length > 0 ? (
              <>
                ...
                <div className="youtube_VideoGrid">
                  {liveVideos.map((v) => <VideoCard key={v.id} video={v} isUploaded={dbVideos.some((d) => d.id === v.id)} />)}
                </div>
              </>
            ) : (
              <div style={{ textAlign: "center", padding: "40px 0", color: "#555" }}>
