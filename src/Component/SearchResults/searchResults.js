...
const SearchResults = () => {
            ...
            pollInterval = setInterval(() => {
              ...
              try {
                ...
                if (state === 0 && autoplayRef.current) {
                  ...
                  if (n < resultsRef.current.length) openVideo(resultsRef.current[n], n);
                }
              } catch (e) {}
            }, 1000);
          },
  ...
  const fetchAll = async (q) => {
    ...
    // ✅ FIX 1: Safe profile search — handles null user/username
    ...
    // ✅ FIX 2: Safe reels search — already uses ?. on all fields
    ...
    // ✅ FIX 3: Safe local videos search — handles null title/channel
  ...
  const fetchYoutube = async (q) => {
    if (!ytCache[`_seen_${q}`]) ytCache[`_seen_${q}`] = new Set();
          ...
          {selectedVideo ? (
            ...
            <div style={{ display: "flex", gap: "24px", padding: "20px 24px", maxWidth: "1600px", margin: "0 auto", flexWrap: "wrap" }}>
              ...
              <div style={{ flex: "1 1 0", minWidth: 0, width: "100%" }}>
                <div style={{ borderRadius: "12px", overflow: "hidden", background: "#000" }}>
                  <div id="yt-player-container" style={{ width: "100%", height: isMobile ? "220px" : "500px" }}>
                    <div id="yt-player" />
                  </div>
                </div>
                  ...
                  <>
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "12px", marginTop: "12px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                        <img src={`https://ui-avatars.com/api/?name=${encodeURIComponent(currentItem.snippet.channelTitle)}&background=random&size=40`} alt="ch" style={{ width: "40px", height: "40px", borderRadius: "50%" }} />
                        <div>
                      ...
                      <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                        <div style={{ display: "flex", background: "#272727", borderRadius: "20px", overflow: "hidden" }}>
                          <button onClick={() => { setLiked(!liked); if (disliked) setDisliked(false); }} style={{ background: liked ? "#3ea6ff22" : "transparent", border: "none", color: liked ? "#3ea6ff" : "white", padding: "8px 16px", cursor: "pointer", fontSize: "14px", borderRight: "1px solid #3a3a3a" }}>👍 1.1K</button>
                          <button onClick={() => { setDisliked(!disliked); if (liked) setLiked(false); }} style={{ background: disliked ? "#ff444422" : "transparent", border: "none", color: disliked ? "#ff4444" : "white", padding: "8px 16px", cursor: "pointer", fontSize: "14px" }}>👎</button>
                        </div>
                        <button onClick={() => { navigator.clipboard.writeText(`https://www.youtube.com/watch?v=${selectedVideo}`); alert("Link copied!"); }} style={{ background: "#272727", border: "none", color: "white", borderRadius: "20px", padding: "8px 16px", cursor: "pointer", fontSize: "14px" }}>🔗 Share</button>
                        <button onClick={() => { setSelectedVideo(null); setSelectedVideoIndex(null); }} style={{ background: "#272727", border: "none", color: "#aaa", borderRadius: "20px", padding: "8px 16px", cursor: "pointer", fontSize: "13px" }}>✕ Close</button>
                      </div>
                    </div>

                    {/* Description */}
                    <div style={{ background: "#272727", borderRadius: "12px", padding: "14px 16px", marginTop: "14px", color: "#ccc", fontSize: "14px", lineHeight: "1.6", cursor: "pointer" }} onClick={() => setShowFullDesc(!showFullDesc)}>
                    ...
                    <div style={{ marginTop: "28px" }}>
                      ...
                      <div style={{ display: "flex", gap: "12px", marginBottom: "24px" }}>
                        <img src="https://athenabpo.com/wp-content/uploads/2016/09/Headshot-Blank-Person-Circle-300x300.gif" alt="user" style={{ width: "36px", height: "36px", borderRadius: "50%", flexShrink: 0 }} />
                        <div style={{ flex: 1 }}>
                          <input
                            type="text"
                            value={comment}
                          ...
                          {comment.trim() && (
                            <div style={{ display: "flex", justifyContent: "flex-end", gap: "8px", marginTop: "8px" }}>
                              ...
                              <button onClick={() => { setComments([{ id: Date.now(), text: comment, user: "You", time: "Just now", likes: 0 }, ...comments]); setComment(""); }} style={{ background: "#3ea6ff", border: "none", color: "black", borderRadius: "20px", padding: "6px 16px", cursor: "pointer", fontWeight: "700", fontSize: "14px" }}>Comment</button>
                            </div>
                          )}
                        </div>
                      </div>
                      {comments.map((c) => (
                        <div key={c.id} style={{ display: "flex", gap: "12px", marginBottom: "20px" }}>
                          <img src={`https://ui-avatars.com/api/?name=${encodeURIComponent(c.user)}&background=random&size=36`} alt={c.user} style={{ width: "36px", height: "36px", borderRadius: "50%", flexShrink: 0 }} />
                          <div>
              ...
              <div style={{ width: isMobile ? "100%" : "402px", flexShrink: 0, position: isMobile ? "relative" : "sticky", top: "70px", height: isMobile ? "auto" : "calc(100vh - 90px)", overflowY: "auto", scrollbarWidth: "thin", scrollbarColor: "#555 transparent" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: "8px", marginBottom: "12px" }}>
                  <span style={{ color: "#aaa", fontSize: "13px" }}>Autoplay</span>
                  <div onClick={() => setAutoplay(!autoplay)} style={{ width: "42px", height: "24px", background: autoplay ? "#ff0000" : "#555", borderRadius: "12px", cursor: "pointer", position: "relative", transition: "background 0.3s" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                  {relatedVideos.map((item) => {
                    ...
                    return (
                      <div key={item.id.videoId} onClick={() => openVideo(item, realIndex)} style={{ display: "flex", gap: "8px", cursor: "pointer", borderRadius: "8px", padding: "4px", transition: "background 0.2s" }} onMouseEnter={(e) => (e.currentTarget.style.background = "#1e1e1e")} onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}>
                        <div style={{ position: "relative", flexShrink: 0, width: "168px", height: "94px", borderRadius: "8px", overflow: "hidden" }}>
                          <img src={item.snippet.thumbnails.medium.url} alt={item.snippet.title} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                        </div>
                        <div style={{ flex: 1, minWidth: 0, paddingTop: "2px" }}>
                          <div style={{ color: "white", fontSize: "13px", fontWeight: "600", lineHeight: "1.4", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden", marginBottom: "4px" }}>{item.snippet.title}</div>
                          <div style={{ color: "#aaa", fontSize: "12px", marginBottom: "2px" }}>{item.snippet.channelTitle}</div>
                          <div style={{ color: "#aaa", fontSize: "12px" }}>{new Date(item.snippet.publishedAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</div>
                        </div>
                      </div>
                    );
                  })}
            ...
            <div style={{ padding: "20px" }}>
                ...
                <div style={{ fontSize: "15px", color: "#aaa", marginBottom: "14px" }}>
                  ...
                  {youtubeResults.length > 0 && (
                    <span style={{ marginLeft: "8px", color: "#555", fontSize: "13px" }}>
                      — {youtubeResults.length} YouTube videos{postResults.length > 0 ? `, ${postResults.length} posts` : ""}
                    </span>
                  )}
                </div>
                ...
                {hasAnyResults && (
                  <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                    {TAB_LIST.map((tab) => {
                      ...
                      return (
                        <button key={tab} onClick={() => setActiveTab(tab)} style={{ padding: "6px 16px", borderRadius: "20px", border: "none", cursor: "pointer", fontSize: "13px", fontWeight: "600", background: activeTab === tab ? "white" : "#272727", color: activeTab === tab ? "black" : "white" }}>
                          {TAB_LABELS[tab]}{tab !== "all" && TAB_COUNTS[tab] > 0 ? ` (${TAB_COUNTS[tab]})` : ""}
                        </button>
                      );
                    })}
                  </div>
                ...
                <div style={{ marginBottom: "40px" }}>
                  ...
                  <div style={{ display: "flex", gap: "16px", flexWrap: "wrap" }}>
                    {profileResults.map((profile) => (
                      <Link key={profile.username} to={`/user/${profile.username}`} style={{ textDecoration: "none" }}>
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "8px", background: "#1a1a1a", borderRadius: "12px", padding: "16px 24px", cursor: "pointer", transition: "background 0.2s", minWidth: "120px" }} onMouseEnter={(e) => (e.currentTarget.style.background = "#272727")} onMouseLeave={(e) => (e.currentTarget.style.background = "#1a1a1a")}>
                          <img src={profile.profilePic} alt={profile.user} style={{ width: "64px", height: "64px", borderRadius: "50%", objectFit: "cover", border: "2px solid #333" }} />
              ...
              {(activeTab === "all" || activeTab === "reels") && reelResults.length > 0 && (
                <div style={{ marginBottom: "40px" }}>
                  <h2 style={{ fontSize: "15px", color: "#aaa", marginBottom: "12px", fontWeight: "600" }}>🎞️ Reels</h2>
                  <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
                    {reelResults.map((reel) => (
                      <Link key={reel.id} to={`/reels/${reel.id}`} style={{ textDecoration: "none" }}>
                        <div style={{ width: "160px", borderRadius: "12px", overflow: "hidden", background: "#1a1a1a", cursor: "pointer" }} onMouseEnter={(e) => (e.currentTarget.style.background = "#272727")} onMouseLeave={(e) => (e.currentTarget.style.background = "#1a1a1a")}>
                          <video src={reel.video} poster={reel.thumbnail} style={{ width: "100%", height: "280px", objectFit: "cover", display: "block" }} muted playsInline onMouseEnter={(e) => e.target.play()} onMouseLeave={(e) => { e.target.pause(); e.target.currentTime = 0; }} />
                          <div style={{ padding: "8px" }}>
                            <div style={{ color: "white", fontSize: "12px", fontWeight: "600", overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>{reel.caption}</div>
                            <div style={{ color: "#aaa", fontSize: "11px", marginTop: "4px" }}>@{reel.username}</div>
                          </div>
                        </div>
                      </Link>
                    ))}
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "16px" }}>
                    {localVideoResults.map((video) => (
                      <Link key={video.id} to={`/video/${video.id}`} style={{ textDecoration: "none" }}>
                        <div style={{ cursor: "pointer" }} onMouseEnter={(e) => (e.currentTarget.querySelector(".lthumb").style.transform = "scale(1.03)")} onMouseLeave={(e) => (e.currentTarget.querySelector(".lthumb").style.transform = "scale(1)")}>
                          <div style={{ borderRadius: "12px", overflow: "hidden", position: "relative" }}>
                            <img className="lthumb" src={video.thumbnail} alt={video.title} style={{ width: "100%", aspectRatio: "16/9", objectFit: "cover", display: "block", transition: "transform 0.3s" }} />
                            <div style={{ position: "absolute", bottom: "6px", right: "8px", background: "rgba(0,0,0,0.8)", color: "white", fontSize: "12px", padding: "2px 6px", borderRadius: "4px" }}>{video.duration}</div>
                          </div>
                          <div style={{ display: "flex", gap: "10px", padding: "10px 4px" }}>
                            <img src={`https://api.dicebear.com/7.x/initials/svg?seed=${video.channel}`} alt={video.channel} style={{ width: "36px", height: "36px", borderRadius: "50%", flexShrink: 0 }} />
                            <div style={{ flex: 1 }}>
                              <div style={{ color: "white", fontWeight: "600", fontSize: "13px", lineHeight: "1.4", marginBottom: "4px", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{video.title}</div>
                ...
                <div style={{ marginBottom: "40px" }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "16px" }}>
                    {postResults.map((post, i) => (
                      <div key={i} style={{ background: "#272727", borderRadius: "12px", overflow: "hidden", cursor: "pointer", transition: "transform 0.2s" }} onMouseEnter={(e) => (e.currentTarget.style.transform = "translateY(-2px)")} onMouseLeave={(e) => (e.currentTarget.style.transform = "translateY(0)")}>
                ...
                <div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "16px" }}>
                    {youtubeResults.map((item, index) => (
                      <div key={item.id.videoId} onClick={() => openVideo(item, index)} style={{ cursor: "pointer" }} onMouseEnter={(e) => (e.currentTarget.querySelector(".thumb").style.transform = "scale(1.03)")} onMouseLeave={(e) => (e.currentTarget.querySelector(".thumb").style.transform = "scale(1)")}>
                        <div style={{ borderRadius: "12px", overflow: "hidden" }}>
                          <img className="thumb" src={item.snippet.thumbnails.medium.url} alt={item.snippet.title} style={{ width: "100%", aspectRatio: "16/9", objectFit: "cover", display: "block", transition: "transform 0.3s" }} />
                        </div>
                        <div style={{ display: "flex", gap: "10px", padding: "10px 4px" }}>
                          <img src={`https://ui-avatars.com/api/?name=${encodeURIComponent(item.snippet.channelTitle)}&background=random&size=36`} alt="ch" style={{ width: "36px", height: "36px", borderRadius: "50%", flexShrink: 0 }} />
                          <div style={{ flex: 1 }}>
                            <div style={{ color: "white", fontWeight: "600", fontSize: "13px", lineHeight: "1.4", marginBottom: "4px", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{item.snippet.title}</div>
                            <div style={{ color: "#aaa", fontSize: "12px", marginBottom: "2px" }}>{item.snippet.channelTitle}</div>
                            <div style={{ color: "#aaa", fontSize: "12px" }}>{new Date(item.snippet.publishedAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</div>
                          </div>
                        </div>
