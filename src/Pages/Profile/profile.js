...
const ProfilePostCard = ({ post, isOwner, onDelete }) => {
  ...
  const itemStyle = (idx, count) => {
    ...
    if (count === 1) return { ...base, cursor: "zoom-in" };
    return base;
  };
    ...
    <>
      ...
      <div style={{ background: "#1a1a1a", border: "1px solid #2a2a2a", borderRadius: "12px", padding: "16px", marginBottom: "14px", position: "relative" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "12px" }}>
          ...
          {isOwner && (
            <button onClick={() => onDelete(post.id)} title="Delete post" style={{ marginLeft: "auto", background: "rgba(200,0,0,0.15)", border: "1px solid rgba(200,0,0,0.3)", color: "#ff6666", borderRadius: "8px", padding: "4px 10px", fontSize: "12px", cursor: "pointer" }}>🗑️ Delete</button>
          )}
        </div>
            ...
            {images.slice(0, 4).map((url, idx) => (
              <div key={idx} style={itemStyle(idx, Math.min(images.length, 4))} onClick={() => setLightboxData({ images, startIndex: idx })}>
                ...
                {idx === 3 && images.length > 4 && (
                  <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontSize: "26px", fontWeight: "800" }}>+{images.length - 4}</div>
                )}
              </div>
            ))}
          </div>
          ...
          <div style={{ marginTop: "12px", display: "flex", flexDirection: "column", gap: "8px" }}>
            {post.comments.map((c) => (
              <div key={c.id} style={{ display: "flex", gap: "8px", alignItems: "flex-start" }}>
                <div style={{ width: "28px", height: "28px", borderRadius: "50%", background: "#333", display: "flex", alignItems: "center", justifyContent: "center", color: "#ccc", fontSize: "11px", fontWeight: "700", flexShrink: 0 }}>{(c.username || "?").slice(0, 2).toUpperCase()}</div>
...
const SubscribersModal = ({ channelUsername, onClose }) => {
  ...
  useEffect(() => {
    const fetchSubscribers = async () => {
      ...
      setSubscribers(data.map((s) => {
        ...
        let displayName;
        if (!isUuid) displayName = s.subscriber_id;
        else if (idToUsername[s.subscriber_id]) displayName = idToUsername[s.subscriber_id];
        else displayName = `User ${s.subscriber_id.slice(0, 8)}`;
  ...
  return (
    <div onClick={(e) => e.target === e.currentTarget && onClose()} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.88)", zIndex: 999999, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "#1a1a1a", borderRadius: "16px", width: "100%", maxWidth: "420px", maxHeight: "75vh", display: "flex", flexDirection: "column", border: "1px solid #2a2a2a", overflow: "hidden" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "20px 24px 16px", borderBottom: "1px solid #2a2a2a" }}>
          <div>
            <h2 style={{ color: "white", margin: 0, fontSize: "18px", fontWeight: "700" }}>👥 Subscribers</h2>
            <p style={{ color: "#888", margin: "2px 0 0", fontSize: "13px" }}>{subscribers.length} {subscribers.length === 1 ? "person" : "people"} subscribed</p>
          </div>
          <button onClick={onClose} style={{ background: "rgba(255,255,255,0.08)", border: "none", color: "white", width: "36px", height: "36px", borderRadius: "50%", cursor: "pointer", fontSize: "16px", display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
        </div>
        <div style={{ overflowY: "auto", flex: 1, padding: "12px 16px" }}>
          {loading ? (
            <div style={{ display: "flex", justifyContent: "center", padding: "40px 0" }}>
              <div style={{ width: "32px", height: "32px", border: "3px solid #333", borderTop: "3px solid #ff4444", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
            </div>
            ...
            subscribers.map((sub, idx) => (
              <div key={idx} style={{ display: "flex", alignItems: "center", gap: "12px", padding: "10px 8px", borderRadius: "10px", transition: "background 0.15s", cursor: "default" }} onMouseEnter={(e) => e.currentTarget.style.background = "#262626"} onMouseLeave={(e) => e.currentTarget.style.background = "transparent"}>
                <div style={{ width: "40px", height: "40px", borderRadius: "50%", background: "linear-gradient(135deg, #ff4444, #cc0000)", display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontWeight: "700", fontSize: "14px", flexShrink: 0, textTransform: "uppercase" }}>{(sub.displayName || "?").slice(0, 2).toUpperCase()}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ color: "white", fontWeight: "600", fontSize: "14px" }}>{sub.displayName}</div>
                  <div style={{ color: "#666", fontSize: "12px" }}>Subscribed {timeAgo(sub.created_at)}</div>
                </div>
              </div>
            ))
          )}
...
const LogoutModal = ({ onConfirm, onCancel }) => (
  <div
    ...
    style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 999999, display: "flex", alignItems: "center", justifyContent: "center" }}
  >
    <div style={{ background: "#212121", borderRadius: "16px", padding: "32px", width: "100%", maxWidth: "340px", border: "1px solid #555", display: "flex", flexDirection: "column", gap: "16px", textAlign: "center" }}>
      <div style={{ fontSize: "40px" }}>👋</div>
      <h2 style={{ color: "white", margin: 0, fontSize: "18px" }}>Log out?</h2>
      <p style={{ color: "#aaa", fontSize: "14px", margin: 0 }}>You'll need to sign in again to access your account.</p>
...
const Profile = ({ sideNavbar }) => {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      ...
      if (lsUsername && matchesKey(lsUsername, key)) {
        const { data: ownerProfile } = await supabase.from("profiles").select("id, banner_pic, profile_pic, about").eq("username", lsUsername).maybeSingle();
        ...
        setUser({ id: ownerProfile?.id || lsUserId, name: lsUsername, handle: `@${lsUsername}`, profilePic, about, email: lsEmail, isOwner: true, bannerPic });
      } else {
        ...
        if (authUsername && matchesKey(authUsername, key)) {
          const { data: ownerProfile } = await supabase.from("profiles").select("id, banner_pic, profile_pic, about").eq("username", authUsername).maybeSingle();
          ...
          setUser({ id: ownerProfile?.id || authUser.id, name: authUsername, handle: `@${authUsername}`, profilePic, about, email: authUser.email, isOwner: true, bannerPic });
          localStorage.setItem("username", authUsername);
          if (profilePic) localStorage.setItem("profilePic", profilePic);
          if (about) localStorage.setItem("about", about);
        } else {
          let foundUser = null;
          const { data: profileRow } = await supabase.from("profiles").select("id, username, banner_pic, profile_pic, about").eq("username", key).maybeSingle();
          if (profileRow) {
            foundUser = { id: profileRow.id, name: profileRow.username, handle: `@${profileRow.username}`, profilePic: profileRow.profile_pic || `https://ui-avatars.com/api/?name=${encodeURIComponent(profileRow.username)}&background=ff0000&color=fff&size=120`, about: profileRow.about || `${profileRow.username}'s channel`, bannerPic: profileRow.banner_pic || null, isOwner: false };
          }
          if (!foundUser) {
      ...
      if (postsData) {
        ...
        setUserPosts(postsData.map((p) => ({ ...p, myReaction: p.post_reactions?.find((r) => r.username === currentUser)?.type || null, reactionCounts: p.post_reactions?.reduce((acc, r) => { acc[r.type] = (acc[r.type] || 0) + 1; return acc; }, {}), comments: (p.post_comments || []).sort((a, b) => new Date(a.created_at) - new Date(b.created_at)) })));
      }
      ...
      if (currentUser && currentUser.toLowerCase() !== key) {
        const { data: subCheckById } = currentUserId ? await supabase.from("subscriptions").select("id").eq("subscriber_id", currentUserId).eq("subscribed_to", key).maybeSingle() : { data: null };
  ...
  // ── FIXED logout — clears storage, signs out, navigates home ──
  const handleLogout = async () => {
    ...
    navigate("/");
    // Reload so App.jsx re-initialises with no user
  ...
  // ── FIXED: upsert by id (not username) to prevent duplicate rows ──
  const handleSaveProfile = async () => {
    ...
    const uid      = user.id || userId || localStorage.getItem("userId");

    localStorage.setItem("username",    newName);
    localStorage.setItem("channelName", newName);
    localStorage.setItem("about",       newAbout);
    localStorage.setItem("profilePic",  newPic);

    if (uid) {
      // Upsert by PRIMARY KEY (id) — this is correct
      await supabase.from("profiles").upsert(
        { id: uid, username: newName, profile_pic: newPic, about: newAbout, banner_pic: user.bannerPic || null },
        { onConflict: "id" }
      );
    } else {
      // Fallback: update by username if no id available
      await supabase.from("profiles").update({ username: newName, profile_pic: newPic, about: newAbout }).eq("username", user.name);
    }
    ...
    <div className="profile">
      ...
      <div className={sideNavbar ? "profile_page" : "profile_page_inactive"}>
        ...
        >
          ...
          <input
            type="file" id="bannerInput" accept="image/*" style={{ display: "none" }}
            onChange={async (e) => {
              ...
              try {
                ...
                const uid = user.id || userId || localStorage.getItem("userId");
                if (uid) {
                  await supabase.from("profiles").upsert({ id: uid, username: localStorage.getItem("username"), banner_pic: url, profile_pic: localStorage.getItem("profilePic") || user.profilePic, about: localStorage.getItem("about") || user.about }, { onConflict: "id" });
                }
        ...
        <div className="profile_top_section">
          <div className="profile_top_section_profile">
            <img className="profile_top_section_img" src={user.profilePic} alt={user.name} onError={(e) => { e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(user.name)}&background=ff0000&color=fff&size=120&length=2`; }} />
          </div>
          <div className="profile_top_section_About">
            <div className="profile_top_section_About_Name">{user.name}</div>
            <div className="profile_top_section_info">
              {user.handle} ·{" "}
              <button onClick={() => setShowSubscribersModal(true)} title="View subscribers" style={{ background: "none", border: "none", padding: 0, color: "inherit", fontSize: "inherit", cursor: "pointer", textDecoration: "underline", textDecorationStyle: "dotted", textUnderlineOffset: "3px" }}>
              ...
              {user.isOwner ? (
                <>
                  ...
                  <button
                    ...
                    onTouchEnd={(e) => { e.preventDefault(); setShowLogoutModal(true); }}
                    style={{ background: "rgba(200,0,0,0.12)", color: "#ff6666", border: "1px solid rgba(200,0,0,0.35)", borderRadius: "20px", padding: "8px 20px", fontSize: "14px", fontWeight: "600", cursor: "pointer", display: "flex", alignItems: "center", gap: "6px" }}
                  >
                    🚪 Logout
                  </button>
                </>
                ...
                <button
                  ...
                  disabled={subLoading}
                  style={{ background: isSubscribed ? "#272727" : "#ff0000", color: "white", border: isSubscribed ? "1px solid #555" : "none", borderRadius: "20px", padding: "8px 24px", fontSize: "14px", fontWeight: "600", cursor: subLoading ? "not-allowed" : "pointer", transition: "all 0.2s", opacity: subLoading ? 0.7 : 1, display: "flex", alignItems: "center", gap: "6px" }}
                >
                  {subLoading
                    ? <span style={{ display: "flex", alignItems: "center", gap: "6px" }}><span style={{ width: "14px", height: "14px", border: "2px solid rgba(255,255,255,0.4)", borderTop: "2px solid white", borderRadius: "50%", animation: "spin 0.7s linear infinite", display: "inline-block" }} />{isSubscribed ? "Unsubscribing..." : "Subscribing..."}</span>
            ...
            : <div className="profileVideos">
                {allUserReels.map((reel) => (
                  <div key={reel.id} style={{ position: "relative" }}>
                    <div className="profileVideo_block" style={{ cursor: "pointer" }} onClick={() => navigate("/reels", { state: { clickedReel: { ...reel, user: reel.user || user.name, username: reel.username || key, profilePic: reel.profilePic || user.profilePic, likes: reel.likes || 0 } } })}>
      ...
      {/* Edit Profile Modal */}
      {showEditProfile && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.85)", zIndex: 999999, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={(e) => e.target === e.currentTarget && setShowEditProfile(false)}>
          <div style={{ background: "#212121", borderRadius: "16px", padding: "32px", width: "100%", maxWidth: "440px", border: "1px solid #333", display: "flex", flexDirection: "column", gap: "16px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
              <img src={editPic || user.profilePic} alt="preview" style={{ width: "72px", height: "72px", borderRadius: "50%", objectFit: "cover", border: "2px solid #ff0000" }} onError={(e) => { e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(editName || user.name)}&background=ff0000&color=fff&size=72`; }} />
              <div style={{ flex: 1 }}>
            ...
            <div>
              ...
              <input type="file" accept="image/*" onChange={async (e) => { const file = e.target.files[0]; if (!file) return; setEditLoading(true); const formData = new FormData(); formData.append("file", file); formData.append("upload_preset", "youtube-clone"); try { const res = await fetch("https://api.cloudinary.com/v1_1/dwoqk0yue/image/upload", { method: "POST", body: formData }); const data = await res.json(); setEditPic(data.secure_url); } catch { alert("Upload failed. Try again."); } setEditLoading(false); }} style={{ color: "#aaa", fontSize: "13px" }} />
      ...
      {deleteTarget && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.85)", zIndex: 999999, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={(e) => e.target === e.currentTarget && setDeleteTarget(null)}>
