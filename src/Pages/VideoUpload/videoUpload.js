...
const VideoUpload = () => {
  ...
  const [provider, setProvider]           = useState("cloudinary"); // ← NEW
  ...
  const [archiveItemId, setArchiveItemId] = useState(""); // ← NEW
  ...
  const uploadToArchive = async (file) => {
    ...
    // Upload using Internet Archive S3-like API
    ...
    return new Promise((resolve, reject) => {
      ...
      xhr.setRequestHeader("x-archive-meta-mediatype",
        uploadMode === "reel" ? "movies" : "movies"
      );
      xhr.setRequestHeader("x-archive-meta-title",
        inputField.title || file.name
      );
      xhr.setRequestHeader("x-archive-meta-description",
      ...
      xhr.setRequestHeader("x-archive-meta-licenseurl",
  ...
  const uploadVideo = async (e) => {
    ...
    // Warn if Supabase selected for large file
    ...
    try {
      ...
      // Auto-capture thumbnail if not manually set
    ...
    try {
      if (uploadMode === "video") {
        const { error: videoError } = await supabase.from("videos").insert([{
          ...
          storage_provider: provider, // ← tracks which provider was used
        }]);
          ...
          <div className="upload_success_screen">
            ...
            {provider === "archive" && archiveItemId && (
              <a
                ...
                rel="noreferrer"
                style={{
                  color: "#3ea6ff", fontSize: "13px", marginBottom: "12px",
        ...
        <div style={{ marginBottom: "16px" }}>
          ...
          {provider === "archive" && (
            <div style={{
              ...
              borderRadius: "8px", fontSize: "12px", color: "#3ea6ff",
            }}>
              🏛️ <strong>Internet Archive</strong> — Free unlimited storage for movies and videos.
              Requires <code>REACT_APP_ARCHIVE_ACCESS</code> and <code>REACT_APP_ARCHIVE_SECRET</code> in your .env file.
          ...
          {provider === "supabase" && (
            <div style={{
              ...
              🟢 <strong>Supabase Storage</strong> — Best for small videos under 50MB. Fast and reliable.
            </div>
          )}
          {provider === "cloudinary" && (
            <div style={{
              ...
              borderRadius: "8px", fontSize: "12px", color: "orange",
            }}>
